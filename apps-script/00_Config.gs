var RDR_SHEETS={
  ...
  LegCrews:['leg_crew_id','leg_id','crew_id','joining','notes','updated_by','updated_at'],Links:['link_id','label','url','thumbnail_url','source','created_by','created_at','updated_at','version','status'],Activity:['activity_id','op_id','person_id','crew_id','entity_type','entity_id','action','summary','created_at','device_id'],ProcessedOps:['op_id','device_id','received_at','result_json'],Config:['key','value','group','sort_order','active']
};
var RDR_IDS={item:'item_id',personal_check:'personal_check_id',commitment:'commitment_id',task:'task_id',trip_leg:'leg_id',link:'link_id'};
var RDR_COLLECTIONS={item:'Items',personal_check:'PersonalChecks',commitment:'Commitments',task:'Tasks',trip_leg:'TripLegs',link:'Links'};
